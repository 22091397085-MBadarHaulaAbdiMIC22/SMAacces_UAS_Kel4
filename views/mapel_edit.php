...
<?php
$id=$_GET['id'];
$ambil=  mysqli_query($koneksi, "SELECT * FROM mapel WHERE id='$id'") or die ("SQL Edit error");
        ...
        <div class="col-xs-12">
            <div class="panel panel-success">
                <div class="panel-heading">
                    <h3 class="panel-title">Update Data Mata Pelajaran</h3>
                </div>
                <div class="panel-body">
                    <!--membuat form untuk tambah data-->
                     <form class="form-horizontal" action="" method="post">
                        <div class="form-group">
                            <label for="kd_mapel" class="col-sm-3 control-label">Kode Mata Pelajaran</label>
                            <div class="col-sm-9">
                                <input type="text" name="kd_mapel" value="<?=$data['kd_mapel']?> "class="form-control" id="inputPassword3" placeholder="Inputkan Kode Mata Pelajaran" required>
                            </div>
                        </div>
                        <div class="form-group">
                            <label for="mapel_nama" class="col-sm-3 control-label">Nama Mata Pelajaran</label>
                            <div class="col-sm-9">
                                <input type="text" name="mapel_nama" value="<?=$data['mapel_nama']?> "class="form-control" id="inputPassword3" placeholder="Inputkan Nama Mata Pelajaran" required>
                            </div>
                        </div>
                        ...
                        <div class="form-group">
                            <div class="col-sm-offset-3 col-sm-9">
                                <button type="submit" class="btn btn-success">
                                    <span class="fa fa-save"></span>Update Data Mata Pelajaran</button>
                            </div>
                        </div>
                    </form>
                ...
                <div class="panel-footer">
                    <a href="?page=mapel&actions=tampil" class="btn btn-danger btn-sm">
...
if($_POST){
    ...
    //buat sql
    $sql="UPDATE mapel SET kd_mapel='$kd_mapel' , mapel_nama='$mapel_nama'
    WHERE id ='$id'"; 
    $query=  mysqli_query($koneksi, $sql) or die ("SQL Edit MHS Error");
    if ($query){
        echo "<script>window.location.assign('?page=mapel&actions=tampil');</script>";
    }else{
